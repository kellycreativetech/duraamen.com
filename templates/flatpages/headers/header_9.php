...
if ( !function_exists( 'brewery_template_header_9_theme_setup' ) ) {
	add_action( 'brewery_action_before_init_theme', 'brewery_template_header_9_theme_setup', 1 );
	function brewery_template_header_9_theme_setup() {
		brewery_add_template(array(
			'layout' => 'header_9',
			'mode'   => 'header',
			'title'  => esc_html__('Header 9', 'brewery'),
			'icon'   => brewery_get_file_url('templates/headers/images/9.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'brewery_template_header_9_output' ) ) {
	function brewery_template_header_9_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_2 top_panel_style_9 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_2 top_panel_inner_style_9 top_panel_position_<?php echo esc_attr(brewery_get_custom_option('top_panel_position')); ?>">
